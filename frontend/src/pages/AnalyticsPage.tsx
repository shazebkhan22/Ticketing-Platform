...
import type {
  ProjectAnalyticsPriorityCount,
  ProjectAnalyticsStatusCount,
} from "@/types/project";
import {
...
export function AnalyticsPage() {
  ...
  const { data: projectData, isLoading: isProjectLoading } = useProjectAnalytics();
  ...
  const projectPriorityData = toPieData<ProjectAnalyticsPriorityCount>(
    projectData?.byPriority,
    (r) => r.priority
  );
  const projectStatusData = toPieData<ProjectAnalyticsStatusCount>(
    projectData?.byStatus,
    (r) => STATUS_TO_CONFIG_KEY[r.status]
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <h2 className="text-lg font-semibold text-neutral-800">Project Analytics</h2>
        <p className="text-sm text-neutral-500">
          Track project delivery performance across the same lifecycle dimensions.
        </p>
      </div>
      ...
      {isProjectLoading ? (
        ...
        <div className="grid gap-6 md:grid-cols-2">
            ...
            <CardContent>
              <ChartContainer config={VOLUME_CONFIG} className="h-64 w-full">
                <AreaChart data={projectData?.monthly ?? []}>
                  ...
                  <ChartTooltip content={<ChartTooltipContent />} />
                  <defs>
                    <linearGradient id="fillProjectCreated" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="var(--color-created)" stopOpacity={0.8} />
                      <stop offset="95%" stopColor="var(--color-created)" stopOpacity={0.1} />
                    </linearGradient>
                    <linearGradient id="fillProjectClosed" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="var(--color-closed)" stopOpacity={0.8} />
                      <stop offset="95%" stopColor="var(--color-closed)" stopOpacity={0.1} />
                    </linearGradient>
                  </defs>
          ...
          <Card>
            ...
            <CardContent>
              <ChartContainer config={ACCOUNT_MANAGER_CONFIG} className="h-64 w-full">
                <BarChart data={projectData?.byAccountManager ?? []}>
                  <CartesianGrid vertical={false} />
                  <XAxis
          ...
          <Card className="md:col-span-2">
            ...
            <CardContent>
              <ChartContainer config={EMPLOYEE_STATUS_CONFIG} className="h-64 w-full">
                <BarChart accessibilityLayer data={projectData?.byEmployee ?? []}>
                  <CartesianGrid vertical={false} />
                  <XAxis
                    dataKey="employee"
                    tickLine={false}
            ...
            <CardFooter className="text-sm text-muted-foreground">
              Total projects currently assigned to each employee, by status
            </CardFooter>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <ChartContainer config={PRIORITY_CONFIG} className="mx-auto h-64 aspect-square">
                <PieChart>
                  <ChartTooltip content={<ChartTooltipContent hideLabel nameKey="key" />} />
                  <Pie data={projectPriorityData} dataKey="count" nameKey="key" innerRadius={55} strokeWidth={4}>
          ...
          <Card>
            ...
            <CardContent>
              <ChartContainer config={STATUS_CONFIG} className="mx-auto h-64 aspect-square">
                <PieChart>
                  <ChartTooltip content={<ChartTooltipContent hideLabel nameKey="key" />} />
                  <Pie data={projectStatusData} dataKey="count" nameKey="key" innerRadius={55} strokeWidth={4}>
